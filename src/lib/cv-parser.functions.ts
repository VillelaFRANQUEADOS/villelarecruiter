...
async function generateObjectWithFallback<T extends z.ZodTypeAny>(
  ...
  for (const attempt of chain) {
    try {
      const { object } = (await generateObject({ model: attempt.model, schema, messages: buildMessages(attempt.model), maxRetries: 0 } as Parameters<typeof generateObject>[0])) as { object: z.infer<T> };
      return object;
    } catch (e) {
      lastError = e;
...
function cleanFileName(name: string) { return name.replace(/\.[^.]+$/, "").replace(/[_\-]+/g, " ").trim() || "Candidato"; }
function normalizePhone(value: string, fallbackText = ""): string { let p = (value || "").replace(/\D/g, ""); if (p.startsWith("55") && p.length > 11) p = p.slice(2); if (p.length === 10 || p.length === 11) return p; return extractPhone(fallbackText); }
function normalizeEmail(value: string, fallbackText = ""): string { const v = (value || "").trim().toLowerCase(); if (/^[\w.+-]+@[\w-]+\.[\w.-]+$/.test(v)) return v; return extractEmail(fallbackText).toLowerCase(); }
...
function normalizeCandidateName(value: string): string {
  ...
  const forbidden = ["curriculo", "currículo", "curriculum", "contato", "contact", "experiencia", "experiência", "resumo", "summary", "objetivo", "objective", "linkedin", "formacao", "formação", "education", "habilidades", "skills", "competencias", "competências", "telefone", "celular", "email", "endereco", "endereço", "perfil", "profile", "sobre mim", "experiência profissional", "status da vaga", "vaga atual", "adequacao com ia", "adequação com ia", "adequacao da ia a vaga", "adequação da ia a vaga", "impressao cv", "impressão cv", "grupo villela"];
  const lower = name.toLowerCase();
  if (forbidden.some((w) => lower.includes(w))) return "";
  if (!words.every((w) => /^[A-Za-zÀ-ÿ'’-]+$/.test(w))) return "";
  return name;
}
function extractNameFromFileName(fileName: string): string { const raw = cleanFileName(fileName).replace(/\b(curriculo|currículo|cv|resume|resum[eé])\b/gi, " ").replace(/\b(arquivo|documento|document)\b/gi, " ").replace(/\s+\d+\s*$/g, "").replace(/\s+/g, " ").trim(); return normalizeCandidateName(raw); }
...
function deterministicExtract(text: string, fileName = ""): Extracted {
  ...
  if (fallback) { cidade = fallback.cidade || cidade; estado = fallback.estado || estado; }
  return { nome: normalizeCandidateName(identity.nome) || normalizeCandidateName(extractName(text)) || extractNameFromFileName(fileName), telefone: identity.telefone || extractPhone(text), email: identity.email || extractEmail(text), cidade, estado };
}
function validateExtractedLocation(cidade: string, estado: string) { return validateCity((cidade || "").trim(), (estado || "").trim()); }
function hasAiProvider(): boolean { return Boolean(process.env.GEMINI_API_KEY || process.env.LOVABLE_API_KEY); }
function needsAiExtraction(extracted: Extracted, location?: ReturnType<typeof validateExtractedLocation>): boolean { return Boolean(!extracted.nome || !extracted.telefone || !extracted.email || !location?.cidade_validada || !location?.estado); }
...
export const parseAndCreateCandidato = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (hasAiProvider() && needsAi && (cvText.replace(/\s/g, "").length >= 30 || images.length > 0)) {
      ...
      } catch (e) { aiFailed = true; aiErrorMsg = e instanceof Error ? e.message : "Erro IA"; }
    }
    ...
    } catch (e) { throw new Error(`Upload Drive falhou: ${e instanceof Error ? e.message : String(e)}`); }
    const { data: inserted, error } = await supabase.from("candidatos").insert({ nome: nomeFinal, telefone: telefoneFinal, email: emailFinal, cidade: location.cidade, estado: location.estado, codigo_ibge: location.codigo_ibge, cidade_validada: location.cidade_validada, cidade_original_extraida: location.cidade_original_extraida, origem_curriculo: origem, observacoes: null, curriculo_url: `drive:${driveFileId}`, recrutador_id: userId, status: "aguardando_contato" }).select().single();
...
async function downloadFromDrive(fileId: string): Promise<{ base64: string; mimeType: string }> { const res = await fetch(`${GATEWAY_DRIVE}/drive/v3/files/${encodeURIComponent(fileId)}?alt=media`, { headers: driveHeaders() }); if (!res.ok) { const text = await res.text().catch(() => ""); if (res.status === 401 || res.status === 403) throw new Error("Conexão com o Google Drive expirou. Reconecte o Google Drive."); if (res.status === 404) throw new Error("Currículo não encontrado no Google Drive."); throw new Error(`Falha ao baixar currículo [${res.status}]: ${text.slice(0, 200)}`); } const mimeType = res.headers.get("content-type")?.split(";")[0] || "application/pdf"; return { base64: Buffer.from(await res.arrayBuffer()).toString("base64"), mimeType }; }
async function getDriveFileName(fileId: string): Promise<string> { try { const res = await fetch(`${GATEWAY_DRIVE}/drive/v3/files/${encodeURIComponent(fileId)}?fields=name`, { headers: driveHeaders() }); if (!res.ok) return ""; const json = (await res.json()) as { name?: string }; return (json.name || "").replace(/^\d+-/, ""); } catch { return ""; } }
...
export const reprocessCandidato = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const fileId = cand.curriculo_url.slice("drive:".length);
    const file = await downloadFromDrive(fileId);
    const filename = await getDriveFileName(fileId);
    let extracted: Extracted;
    if (file.mimeType.includes("pdf")) {
      const pdfText = await extractPdfTextFromBase64(file.base64);
      extracted = deterministicExtract(pdfText, filename);
      if (needsAiExtraction(extracted, validateExtractedLocation(extracted.cidade, extracted.estado)) && hasAiProvider()) {
        extracted = await extractWithAiFromFile(file.base64, file.mimeType, filename);
      }
    } else {
      if (!hasAiProvider()) throw new Error("Reprocessamento de arquivos não-PDF requer um provedor de IA configurado.");
      extracted = await extractWithAiFromFile(file.base64, file.mimeType, filename);
    }
    const nome = normalizeCandidateName(extracted.nome) || cand.nome;
    const telefone = normalizePhone(extracted.telefone);
    const email = normalizeEmail(extracted.email) || null;
    const location = validateExtractedLocation(extracted.cidade, extracted.estado);
    const { data: updated, error } = await supabase.from("candidatos").update({ nome, telefone, email, cidade: location.cidade, estado: location.estado, codigo_ibge: location.codigo_ibge, cidade_validada: location.cidade_validada, cidade_original_extraida: location.cidade_original_extraida }).eq("id", cand.id).select().single();
    if (error) throw new Error(error.message);
    return { candidato: updated };
  });
