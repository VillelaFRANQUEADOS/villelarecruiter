...
function tryCandidatesBeforeUf(
  compact: string,
  index: number,
  uf: string
): { cidade: string; estado: string; codigo_ibge: string | null; cidade_validada: boolean; cidade_original_extraida: string | null } | null {
  ...
  for (let size = words.length; size >= 1; size--) {
    ...
    if (location.cidade_validada && cidade) {
      return { ...location, cidade, estado: location.estado ?? uf };
    }
  }
  return null;
}
...
async function extractPdfTextFromBase64(base64: string): Promise<string> {
  ...
  // No ambiente Node.js (build "legacy"), o pdfjs-dist não tem acesso a Web Workers
  // reais e cai automaticamente para execução na thread principal (fake worker),
  // então não é necessária (nem válida) a opção "disableWorker".
